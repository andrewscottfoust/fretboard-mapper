...
export class FretboardComponent implements OnInit, OnChanges {
  ...
  handleNoteToggle(string: any, note: any, name: any) {
  ...
  initFretMarkers(): void {
    ...
    while (i < this.settings.numFrets - 1) {
      let isMarker = false;
      if (
        this.settings.fretMarkerNums.findIndex((item: any) => item === i) > -1
      ) {
        isMarker = true;
      }
  ...
  initStringsArray() {
    ...
    let curFret = 0;
    let noteStartIdx = 0;
    this.settings.stringNames.forEach((string: any) => {
      curFret = 0;
      const stringNotes = [];
      noteStartIdx = this.scaleNotes.findIndex((item: any) => item === string);
      while (curFret < this.settings.numFrets) {
        const noteName = this.scaleNotes[noteStartIdx];
        const noteObj = {
          noteIdx: noteStartIdx,
          noteName: noteName,
          value: this.settings.selectedNotes.findIndex((item: any) => this.scaleNotes[item] === noteName) > -1 && curFret >= this.settings.fretBoardRange.low && curFret <= this.settings.fretBoardRange.high ? 1 : 0,
  ...
  manualSelectNotes() {
    this.settings.manuallySelectedNotes.forEach((item: any) => {
  ...
  updateChordName($event: any) {
    console.log($event);
    this.chordName = $event.value;
  }

  updateRootName($event: any) {
